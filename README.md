...
Every file (other than the two Pokémon tables — see below) is generated by
[`scripts/generate.py`](scripts/generate.py) directly from Python's stdlib
`codecs`/`encodings` modules — nothing here is hand-transcribed. Run
`python3 scripts/generate.py` to regenerate this whole directory, and
`python3 scripts/generate.py --check` to verify it's up to date and that the
directory contains nothing but generated files, without writing anything.
UTF-8 has no file here since ImHex decodes it algorithmically.

Filenames are the underlying Python codec's own name, since that's what the
generator can derive mechanically — a file's first lines are `-name` (the
properly-capitalized IANA name, since filenames are lowercase-only), where
known `-description`, and `-include <base-name>` wherever the generator finds
that this encoding's entries are a superset of another already-generated
one's (computed by comparing every pair, not a curated list — every
ASCII-transparent encoding ends up including `ascii`, Windows-31J includes
Shift_JIS, and a few single-byte encodings that happen to share a lot of
codepoints include each other), so the rest of the file only lists what it
adds or overrides. Every other name a
codec is known by generates its own file too, containing nothing but a
single `-alias <primary-name>` line, so ImHex can resolve any of them
without scanning full file content. The table below links to each *primary*
file; use its `-name` line as the canonical display name.

The two Pokémon tables are hand-authored game-text encodings with no
algorithmic or library source, so the generator explicitly leaves them alone
(see `HAND_AUTHORED_FILES` in `scripts/generate.py`).
